...
SELECT DEPARTMENT_ID , COUNT(*) -- 각 부서별 사원수
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID
HAVING COUNT(*)>=30;
...
SELECT DEPARTMENT_ID AS 부서아이디 ,TRUNC( AVG(SALARY) ) AS 그룹평균급여
FROM EMPLOYEES
...
--GROUPING() - 그룹절로 생성되면 0반환, ROLLUP 0R CUBE 로 만들어지면 1반환
...
--3
SELECT DEPARTMENT_ID AS 부서,TRUNC(AVG(SALARY)) AS 평균급여
FROM EMPLOYEES
WHERE SALARY >= 1000
GROUP BY DEPARTMENT_ID
...
--5
SELECT DECODE((JOB_ID),NULL,'합계',JOB_ID) AS JOB_ID,SUM(SALARY)      
FROM EMPLOYEES
...
SELECT DECODE(GROUPING(DEPARTMENT_ID),1,'합계',DEPARTMENT_ID) AS DEPARTMENT_ID,
       DECODE((JOB_ID),NULL,'소계',JOB_ID) AS JOB_ID,COUNT(*),
       SUM(SALARY)
FROM EMPLOYEES
GROUP BY ROLLUP(DEPARTMENT_ID , JOB_ID)
ORDER BY SUM(SALARY);
